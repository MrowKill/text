...
fn.initNode = function(options){
    ...
    this.body = (function(){
        var strHTML = '<div class="m-comment">' +
                          '<div class="cmt-form">' +
                              '<textarea class="cmt-text" placeholder="分享你的感受吧！"></textarea>' +
...
fn.showList = (function(){
    ...
    function oneLi(_obj){
        ...
        if(_obj.username == "kang"){
            ...
            headImg = $('.login_success img').attr('src');
//          console.log(headImg);
        }
...
fn.addCmt = function (_btn, _text, _parent) {
    ...
    if (!username) {
        username = $('.login_success span').text();
    }
    email = $.cookie('email');
    if (!email) {
        email = '1414697412@qq.com';
    }
...
fn.doClickResponse = function(_event){
    ...
    if (target.hasClass('response') && target.attr('data-disabled') != 'true') {
        ...
        oDiv.innerHTML = '<textarea class="cmt-text" placeholder="分享你的感受吧！"></textarea>' +
